        ...
        if _bashErrCode != 0:
            ...
            for repoPath in _repoList.splitlines():
                ...
                if (version != "" and baseName == "mysqld"):
                    ...
                    logger.debug("Adding repository : "+repository.name)
...
def start_staging(staged_source, repository, source_config):
    ...
    stagingConn = build_lua_connect_string(
        staged_source.parameters.staging_user, staged_source.parameters.stagingip)
...
#            logger.debug(
#                "We are about to run this restore script : \n"+restore_script)

            # logger.debug("\n\n\nPassed staged source to script is : " +
            ...
            output = result.stdout.strip()
            logger.debug(
                "\n\nThe output after running restore script is : \n"+output)
            error = result.stderr.strip()
            exit_code = result.exit_code
            if exit_code != 0:
                logger.debug("Error is : "+error)
...
#        result = libs.run_bash(
#            staged_source.staged_connection, tbsp_script, environment_vars, check=True)
#        output = result.stdout.strip()
#        error = result.stderr.strip()
#        exit_code = result.exit_code
#        if exit_code != 0:
#            logger.debug(
#                "There was an error while copying tablespace : "+error)
#            raise LinkingException(
#                "Exception in pre-snapshot/tablespace copy:"+error)
#        else:
#            logger.debug("Pre-Snapshot/Restore_DB successful "+output)
...
def linked_status(staged_source, repository, source_config):
    ...
    logger.debug("Running the status script >> "+status_script)
    output = result.stdout.strip()
    logger.debug("The output of the status script is : "+output)
    error = result.stderr.strip()
    exit_code = result.exit_code
    if exit_code != 0:
...
def configure(virtual_source, snapshot, repository):
    ...
    result = libs.run_bash(virtual_source.connection,
                           configure_script, environment_vars, check=True)
    logger.debug(result)
    output = result.stdout.strip()
    exit_code = result.exit_code
    error = result.stderr.strip()
...
def stop_mysql(port, connection, baseDir, user, pwd, host):
    ...
    if(port_stat == Status.ACTIVE):
        ...
        if exit_code != 0:
            logger.debug(
                "There was an error trying to shutdown the database : "+error)
...
def get_port_status(port, connection):
    ...
    output = ""
    try:
        port_status_cmd = "ps -ef | grep -E \"[m]ysqld .*--port=" + \
            myport+"\" | grep -v grep"
...
def runbash(connection, command, environmentVars):
    #    logger.debug("operatins.runbash() connection value >> "+connection)
    logger.debug("operatins.runbash() command value >> "+command)
    # logger.debug("operatins.runbash() environmentVars >> "+environmentVars)
