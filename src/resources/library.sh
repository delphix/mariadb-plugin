...
# library.sh
#
# Library of common MariaDB toolkit functions ... 
#
...
startDatabase() {
   ...
   then 
      ...
      if [[ -z "$ZPSID" ]]
      then
        log "MariaDB Database could not be started."
        terminate "MariaDB Database could not be started.No process running." 3
      fi
      ...
      log "Warning: MariaDB is already running. ABORTING start operation."
   fi
}
