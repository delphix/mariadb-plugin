...
# Create Initial Database 10.3 or below ..
   ...
   #sudo ${MYSQLD}/mysqld --initialize --user=mysql --datadir=${NEW_DATA_DIR} --log-error=${NEW_DATA_DIR}/mysqld.log 1>>${DEBUG_LOG} 2>&1
...
#mkdir -p ${NEW_DATA_DIR} # is created by the mysql_install_db command
mkdir -p ${NEW_LOG_DIR}
mkdir -p ${NEW_TMP_DIR}
...
#
# --connect-expired-password option isn't present in version 10.3.x of MariaDB
...
masklog "Reset Master Command:  ${CMD}"       # TODO REMOVE
eval ${CMD} 1>>${DEBUG_LOG} 2>&1
...
if [ $return_code != 0 ]; then
  terminate "Unable to connect to MariaDB Staging DB after backup ingestion." 7
fi
