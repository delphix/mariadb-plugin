...
log "Creating an initial MariaDB instance ..."
...
CMDFORLOG="${INSTALL_BIN}/mysql ${STAGING_CONN} --connect-expired-password -se \"ALTER USER 'root'@'localhost' IDENTIFIED BY ${STAGINGPASS}; FLUSH PRIVILEGES;\""
masklog "Change Password: ${CMD}"
...
masklog "Validating new connection string ..."
CMD="${INSTALL_BIN}/mysql ${STAGING_CONN} -e \"SELECT @@BASEDIR;\""
CMDFORLOG="${INSTALL_BIN}/mysql ${STAGING_CONN} -e \"SELECT @@BASEDIR;\""
