...
def start_staging(staged_source, repository, source_config):
    logger.debug("In linked.start_staging > Starting Staged DB")
...
def stop_staging(staged_source, repository, source_config):
    logger.debug("In linked.stop_staging > Stopping Staged DB")
...
def linked_pre_snapshot(staged_source, repository, source_config, optional_snapshot_parameters):
    logger.debug("In linked_pre_snapshot > Start ")
    ...
    if optional_snapshot_parameters is not None and optional_snapshot_parameters.resync:
        ...
        logger.debug("Resync is set / initial ingestion")
        pluginops.linked_pre_snapshot(staged_source, repository,
                                      source_config, optional_snapshot_parameters)
    else:
        #    linked.pre_snapshot(staged_source, repository, source_config)
        logger.debug("Normal snapshot")
        pluginops.linked_pre_snapshot(staged_source, repository,
                                      source_config, optional_snapshot_parameters)
...
def linked_post_snapshot(staged_source, repository, source_config, optional_snapshot_parameters):
    logger.debug("In linked_post_snapshot - Start ")
    snapshot = pluginops.linked_post_snapshot(
...
def start(virtual_source, repository, source_config):
    ...
    pluginops.start_mysql(repository.install_path, virtual_source.parameters.base_dir,
                          virtual_source.mounts[0].mount_path, virtual_source.parameters.port, virtual_source.parameters.server_id, virtual_source.connection)
    virtual_status(virtual_source, repository, source_config)
