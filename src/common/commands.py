...
class OSCommand(object):
    ...
    def find_binary_path():
        # add fix to skip /proc search
        # return "find / -xdev ! -path \"/etc/*\" ! -path \"/var/lock/*\" -name mysqld -type f -print 2>&1 | grep -v 'Permission denied'"
        return "find / -xdev ! -path \"/etc/*\" ! -path \"/var/lock/*\" -name mysqld -type f -print 2>&1 | grep -v 'Permission denied'"
...
class DatabaseCommand(object):
    ...
    def stop_mysql(baseDir, vdbConn, pwd, port):
        stop_cmd = "%s/bin/mysqladmin %s'%s' --protocol=TCP --port=%s shutdown" % (
...
if __name__ == "__main__":
    ...
    hostname = "hostname"
    port = "3320"
    print "find_install_path: ", CommandFactory.find_install_path(
        binary_path), "\n"
